...
public class Account {
     ...
     */
    /*
        synchronized出现在实例方法上锁的一定是this，不能挑，这种方式不灵活
        另外：整个方法体都需要同步，可能无故扩大了同步的范围，可能导致程序的效率降低

        那么synchronized用在实例方法上的优点？
            代码写的少了，节俭了
            如果共享的对象就是this，并且需要同步的代码块是整个方法体，就使用这种方法

            如StringBuilder在局域变量效率比StringBuffer效率高！！因为局部变量不存在线程安全问题。
            要根据实际情况来
            如ArrayList是非线程安全的，Vector是线程安全的，底层都是数组
              HashMap，HashSet是非线程安全的，HashTable是线程安全的


     */
    public synchronized void withdraw(double money){ //在实例方法上使用synchronized
